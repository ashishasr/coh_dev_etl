--------------------------------------------------------
--  DDL for Package Body PKG_LOAD_8_CMN_ASSET_CPAL_INFO
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "COH_DEV_T"."PKG_LOAD_8_CMN_ASSET_CPAL_INFO" AS
-------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
    PROCEDURE proc_load_common_asset_cleaned_8_cpal_info IS
    BEGIN
        execute immediate 'truncate table  load_common_asset_cleaned_8_cpal_info';

        COMMIT;
        INSERT INTO load_common_asset_cleaned_8_cpal_info (
            asset_id,
            class_id,
            category_id,
            br1,
            ...
            WITH u AS (
                ...
                FROM
                         load_common_asset_cleaned_1_asset_info a
                    INNER JOIN load_common_asset_cleaned_7_brr_info b ON a.asset_id = b.asset_id
                    INNER JOIN wwo_asset_types_master            c ON a.class_id = c.class_id
                                                           AND a.category_id = c.category_id
                    INNER JOIN wwo_cost_override_master          d ON a.class_id = d.class_id
                                                             AND a.category_id = d.category_id
                    INNER JOIN load_common_asset_cleaned_4_amc_info e ON e.asset_id = b.asset_id
            )
            SELECT
                *
            FROM
        ...
        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            first_action_timeframe_br_1 = pkg_load_8_cmn_asset_cpal_info.get_cpal_1_action_timeframe_br1(br1);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            first_action_timeframe_br_2 = pkg_load_8_cmn_asset_cpal_info.get_cpal_1_action_timeframe_br2(br2);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            first_action_timeframe_br_3 = pkg_load_8_cmn_asset_cpal_info.get_cpal_1_action_timeframe_br3(br3);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            first_action_cost = pkg_load_8_cmn_asset_cpal_info.get_cpal_1_action_cost(class_id, category_id);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            second_action = pkg_load_8_cmn_asset_cpal_info.get_cpal_2_action(no_of_rehab);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            second_action_timeframe_br_1 = pkg_load_8_cmn_asset_cpal_info.get_cpal_2_action_timeframe_br1(no_of_rehab, first_action_timeframe_br_1
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            second_action_timeframe_br_2 = pkg_load_8_cmn_asset_cpal_info.get_cpal_2_action_timeframe_br2(no_of_rehab, first_action_timeframe_br_2
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            second_action_timeframe_br_3 = pkg_load_8_cmn_asset_cpal_info.get_cpal_2_action_timeframe_br3(no_of_rehab, first_action_timeframe_br_3
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            third_action = pkg_load_8_cmn_asset_cpal_info.get_cpal_3_action(no_of_rehab);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            third_action_timeframe_br_1 = pkg_load_8_cmn_asset_cpal_info.get_cpal_3_action_timeframe_br1(no_of_rehab, second_action_timeframe_br_1
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            third_action_timeframe_br_2 = pkg_load_8_cmn_asset_cpal_info.get_cpal_3_action_timeframe_br2(no_of_rehab, second_action_timeframe_br_2
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            third_action_timeframe_br_3 = pkg_load_8_cmn_asset_cpal_info.get_cpal_3_action_timeframe_br3(no_of_rehab, second_action_timeframe_br_3
            , rehab_eul, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            third_action_cost = pkg_load_8_cmn_asset_cpal_info.get_cpal_3_action_cost(class_id, category_id, third_action);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            fourth_action = pkg_load_8_cmn_asset_cpal_info.get_cpal_4_action(no_of_rehab);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            fourth_action_timeframe_br_1 = pkg_load_8_cmn_asset_cpal_info.get_cpal_4_action_timeframe_br1(no_of_rehab, second_action_timeframe_br_1
            , rehab_eul, third_action_timeframe_br_1, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            fourth_action_timeframe_br_2 = pkg_load_8_cmn_asset_cpal_info.get_cpal_4_action_timeframe_br2(no_of_rehab, second_action_timeframe_br_2
            , rehab_eul, third_action_timeframe_br_2, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            fourth_action_timeframe_br_3 = pkg_load_8_cmn_asset_cpal_info.get_cpal_4_action_timeframe_br3(no_of_rehab, second_action_timeframe_br_3
            , rehab_eul, third_action_timeframe_br_3, replacement_eul);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            fourth_action_cost = pkg_load_8_cmn_asset_cpal_info.get_cpal_4_action_cost(class_id, category_id, fourth_action);

        COMMIT;
        UPDATE load_common_asset_cleaned_8_cpal_info
        SET
            timeframe = pkg_load_8_cmn_asset_cpal_info.get_cpal_timeframe(br3);
...
--------------------------------------------------------------------------------
    ...
    BEGIN
        proc_load_common_asset_cleaned_8_cpal_info;
    END;
