--------------------------------------------------------
--  DDL for Package Body PKG_EXECUTE_P5_COMMON_ASSET_COND_Q_MAP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "COH_DEV_T"."PKG_EXECUTE_P5_COMMON_ASSET_COND_Q_MAP" AS

PROCEDURE proc_load_common_asset_condition_q_map IS
    BEGIN
        EXECUTE IMMEDIATE 'truncate table common_asset_condition_q_map';
        COMMIT;
        INSERT INTO common_asset_condition_q_map (
            data_unit,
            class_id,
            class_description,
            raw_condition_q_id,
            ...
            SELECT
                ...
                c.ref_condition_q
            FROM
                wwo_common_asset_condition_set_q_master_key raw1
                LEFT JOIN wwo_segmnt_common_asset_class_master        seg ON seg.wwo_class_id = raw1.class_id
...
---------------------------------------------------------------------------------------------
        ...
        MERGE INTO common_asset_condition_q_map target
        USING (
            SELECT
                class_id,
                class_description,
...
----------------------------------------------------------------------------------------------
  ...
  -- Open cursor to traverse source_table
    ...
    -- Update the source_table with the fetched lookup_value
            UPDATE common_asset_condition_q_map
            SET
...
-----------------------------------------------------------------------------
    PROCEDURE upd_wwo_common_asset_cleaned_no_calc_condition_q IS
    BEGIN
        MERGE INTO wwo_common_asset_cleaned_no_calc_condition_q t
        USING common_asset_condition_q_map s ON ( t.raw_condition_q_id = s.raw_condition_q_id )
        WHEN MATCHED THEN UPDATE
        ...
        COMMIT;
        UPDATE wwo_common_asset_cleaned_no_calc_condition_q
        SET
...
-----------------------------------------------------------------------------
    PROCEDURE proc_load_wwo_common_asset_cleaned_no_calc_condition_q_t IS
    BEGIN
        EXECUTE IMMEDIATE 'truncate table wwo_common_asset_cleaned_no_calc_condition_q_t';
        INSERT INTO wwo_common_asset_cleaned_no_calc_condition_q_t (
            asset_id,
            class_id,
            class_description,
            condition_q1,
        ...
        -- case where we have same condition Q for an asset and one condition q is not populated.

        MERGE INTO wwo_common_asset_cleaned_no_calc_condition_q_t t
        ...
        COMMIT;
        UPDATE wwo_common_asset_cleaned_no_calc_condition_q_t
        SET
            condition_a1 = 0
        WHERE
...
----------------------------------------------------------------------------
    ...
    BEGIN
        proc_load_common_asset_condition_q_map;
        update_source_table;
        upd_wwo_common_asset_cleaned_no_calc_condition_q;
        proc_load_wwo_common_asset_cleaned_no_calc_condition_q_t;
    END;

END PKG_EXECUTE_P5_COMMON_ASSET_COND_Q_MAP;
