--------------------------------------------------------
--  DDL for Package Body PKG_LOAD_7_ASSET_BRR_INFO
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "COH_DEV_T"."PKG_LOAD_7_ASSET_BRR_INFO" AS
...
----------------------------------------
    ...
    PROCEDURE proc_load_unq_asset_cleaned_7_brr_info IS
    BEGIN
        execute immediate 'truncate table load_unq_asset_cleaned_7_brr_info';

        COMMIT;
        INSERT INTO load_unq_asset_cleaned_7_brr_info (
            asset_id,
            cof_score,
            condition_score,
            performance_score,
            ...
            SELECT
                ...
                e.lof_score
            FROM
                     load_unq_asset_cleaned_5_cof_info a
                LEFT JOIN load_unq_asset_cleaned_2_condition_q_info   b ON a.asset_id = b.asset_id
                LEFT JOIN load_unq_asset_cleaned_3_performance_info   c ON a.asset_id = c.asset_id
                LEFT JOIN load_unq_asset_cleaned_4_amc_info           d ON a.asset_id = d.asset_id
                LEFT JOIN load_unq_asset_cleaned_6_perf_cond_lof_info e ON a.asset_id = e.asset_id;

        COMMIT;
        UPDATE load_unq_asset_cleaned_7_brr_info
        SET
            risk_baseline_value = pkg_load_7_asset_brr_info.get_brr_risk_baseline_value(lof_score, cof_score, redundancy_factor_revised
            );

        COMMIT;
        UPDATE load_unq_asset_cleaned_7_brr_info
        SET
            br1 = pkg_load_7_asset_brr_info.get_brr_br1(risk_baseline_value);

        COMMIT;
        UPDATE load_unq_asset_cleaned_7_brr_info
        SET
            br2 = pkg_load_7_asset_brr_info.get_brr_br2(condition_score, br1);

        COMMIT;
        UPDATE load_unq_asset_cleaned_7_brr_info
        SET
            br3 = pkg_load_7_asset_brr_info.get_brr_br3(predicted_cip_year, br2);
    ...
    BEGIN
        proc_load_unq_asset_cleaned_7_brr_info;
    END;
