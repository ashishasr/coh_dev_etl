--------------------------------------------------------
--  DDL for Package Body PKG_LOAD_2_ASSET_CONDITION_Q_INFO
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "COH_DEV_T"."PKG_LOAD_2_ASSET_CONDITION_Q_INFO" AS
...
---------------------------------------------------------------------------------------
    ...
    END;
------------------------------------------------------------------------------------------
    PROCEDURE proc_load_unq_asset_cleaned_2_condition_q_info AS
    BEGIN
        EXECUTE IMMEDIATE 'truncate table load_unq_asset_cleaned_2_condition_q_info';
        COMMIT;
        INSERT INTO load_unq_asset_cleaned_2_condition_q_info (
            asset_id,
            class_id,
            class_description
        )
            SELECT
                ...
                class_description
            FROM
                load_unq_asset_cleaned_1_asset_info;

        COMMIT;
    ------------------------    
        MERGE INTO load_unq_asset_cleaned_2_condition_q_info x
        --USING WWO_CONDITION_SET_Q_MASTER mst ON ( x.class_id = mst.class_id )
        USING wwo_unq_asset_cleaned_no_calc_condition_q_t mst ON (  x.asset_id = mst.asset_id)
        ...
        COMMIT;
        MERGE INTO load_unq_asset_cleaned_2_condition_q_info x
        --USING wwo_unq_asset_cleaned_no_calc mst ON ( x.asset_id = mst.asset_id )
        USING wwo_unq_asset_cleaned_no_calc mst ON ( x.asset_id = mst.asset_id)
        WHEN MATCHED THEN UPDATE
        ...
        COMMIT;
        UPDATE load_unq_asset_cleaned_2_condition_q_info
        SET
            condition_score = pkg_load_2_asset_condition_q_info.get_condition_score(condition_a1, condition_a2, condition_a3, condition_a4
            , condition_a5,
        ...
        UPDATE load_unq_asset_cleaned_2_condition_q_info
        SET
            condition_rating = pkg_load_2_asset_condition_q_info.get_condition_rating(condition_score);
    ...
    BEGIN
        proc_load_unq_asset_cleaned_2_condition_q_info;
    END;
